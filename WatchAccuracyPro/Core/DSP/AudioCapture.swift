...
final class AudioCapture: AudioSource {
    ...
    var clockDriftFactor: Double {
        ...
        guard frames > sampleRate * 5 else { return 1.0 }
        let audioSec = frames / sampleRate
        let hostSec = Double(l.hostTime &- f.hostTime) * Self.machSecondsPerTick
        guard hostSec > 0 else { return 1.0 }
        let factor = hostSec / audioSec
        guard abs(factor - 1.0) < 0.0005 else { return 1.0 }   // ±500ppm 밖 = 측정오류 → 무보정
        return factor
    }
