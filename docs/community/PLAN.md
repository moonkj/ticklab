...
## 9. 운영 관리자 · 콘텐츠 모더레이션 (2026-06 구축)

> 신고 처리·공지·경고·제재·감사·거래금지 등 라이브 운영 시스템. 상세:
> - **운영 관리자**: [`OPERATIONS_ADMIN.md`](OPERATIONS_ADMIN.md) — 운영 ID·대시보드·신고·공지·경고·제재·감사·데이터경계(A/B).
> - **콘텐츠 모더레이션**: [`MODERATION_POLICY.md`](MODERATION_POLICY.md) — 거래 전면 금지·3단계 방어·단계적 제재.

- **구축됨(앱)**: 운영 ID 전환(TickLab 게시+앱아이콘 아바타·상단 배지) · 운영 대시보드(통계 그리드·신고 상세·숨김/삭제·작성자 경고) · 공지(노출기간·수정·하단시트·오늘 하루 안 보기) · 거래금지 자동탐지(작성 차단) · 작성화면 가이드라인 고지 · 신고사유 거래/사기 추가.
- **백엔드 SQL(순서대로 배포)**: `admin_rls.sql`(is_admin·admin_users·닉네임 보호) → `admin_ops.sql`(presence·신고 조회·숨김/삭제·공지·경고) → `admin_enforcement.sql`(제재 단계·감사 로그·정지 게시차단).
- **데이터 경계 불변**: 측정 데이터(A, 온디바이스)는 운영 백엔드(B)로 전송/저장 금지. 운영자도 측정 데이터 접근 불가(Hard Rule #8).
- **미구현(후속)**: 웹 어드민 콘솔(제재 부과·이의신청·감사 조회·세그먼트), CSAM 외부연동, ML 독성분류(P2), 스팸 rate-limit. 약관·가이드라인·연령 17+ 문서화(법무, 출시 전).
